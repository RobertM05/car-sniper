                    ...
                    if title_tag and price_tag and link_tag:
                        ...
                        for sp in spans:
                            ...
                            if 'km' in t.lower() and len(t) > 6:
                                m = re.search('(\\d{4})\\s*-\\s*([\\d\\s]+)\\s*km', t, re.IGNORECASE)
                                if not m:
                                    m = re.search('(\\d{4})\\s+([\\d\\s]+)\\s*km', t, re.IGNORECASE)
                                if m:
                                    car_year = m.group(1)
                                    car_km = m.group(2).replace(' ', '') + ' km'
                                    break
                        page_ads.append({'title': title_tag.get_text(strip=True), 'price': price_tag.get_text(strip=True), 'link': link_href, 'image': image_src, 'subsource': 'Autovit' if is_autovit else 'OLX', 'year': car_year, 'km': car_km})
