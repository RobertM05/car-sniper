...
def scrape_autovit(make: str, model: str, page: int = 1, limit: int = 10, max_pages: int = 5, enrich: bool = False):
    """
    ...
    while len(results) < limit and current_page <= max_pages:
        url = BASE_URL.format(make.lower(), model.lower()) + f"?page={current_page}"
        try:
            ...
            if script and script.string:
                ...
                for idx, elem in enumerate(items):
                    ...
                    if not name or price is None:
                        continue
                    link = detail_links[idx] if idx < len(detail_links) else url
                    ...
                    # Detalii suplimentare din pagina de anunt
                    try:
        ...
        # 2) Completeaza din link-urile ramase (constructie din pagina de detaliu)
        if len(results) < limit:
