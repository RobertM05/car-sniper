...
TARGETS = [
    {"make": "BMW", "model": "Seria 1"},
    {"make": "BMW", "model": "Seria 3"},
    {"make": "BMW", "model": "Seria 4"},
    ...
    {"make": "Audi", "model": "Q7"},
    {"make": "Audi", "model": "Q8"},
    {"make": "Mercedes", "model": "Clasa A"},
    {"make": "Mercedes", "model": "Clasa C"},
    ...
    try:
        ...
        for ad in results:
            try:
                ...
                is_missing_image = not ad.get("image") or "no_thumbnail" in str(
                    ad.get("image")
                )
                if is_missing_image:
                    logging.info(
                        f"🔧 Attempting repair for: {ad.get('title')} (Price: {price_val})"
                    )
                    try:
                        import aiohttp
                        ...
                        async with aiohttp.ClientSession(
                            ...
                            async with sess.get(ad.get("link"), timeout=10) as r:
                                if r.status == 404 or len(str(r.url)) < 30:
                                    logging.info(
                                        f"🗑 Found GHOST AD (404/Redirect): {ad.get('title')}. Deleting..."
                                    )
...
async def run_crawler():
    ...
    while True:
        ...
        try:
            cleaned = car_db_optimizer.deactivate_stale_ads(hours_threshold=96)
