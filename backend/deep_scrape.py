...
BRAND_TIERS = {
    ...
    "tier1": {"pages": 500, "brands": ["Volkswagen", "BMW", "Audi", "Mercedes-Benz"]},
...
async def run_deep_scrape():
    ...
    for brand in BRAND_PAGES:
        ...
        try:
            av_results = await scrape_buckets(
                "Autovit",
                lambda lo, hi: scrape_autovit(
                    make=brand,
                    model="",
                    limit=4000,
                    max_pages=100,
        ...
        # 2. OLX — price buckets
        try:
            olx_results = await scrape_buckets(
                "OLX",
                lambda lo, hi: scrape_olx(
                    query=brand,
                    limit=4000,
                    max_pages=100,
                    min_price=lo,
