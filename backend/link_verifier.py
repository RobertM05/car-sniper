...
async def check_single_link(session, ad):
    try:
        ...
        async with session.get(
            ad.get("link"), timeout=8, allow_redirects=True
        ) as response:
            ...
            if len(final_url) < 35 or (
                "autovit.ro" in original_url and "autovit.ro/anunt/" not in final_url
            ):
                return ad, False
...
async def verify_ads_liveness(ads_list):
    ...
    try:
        async with aiohttp.ClientSession(
            ...
            tasks = [
                check_single_link(session, ad) for ad in ads_to_check if ad.get("link")
            ]
