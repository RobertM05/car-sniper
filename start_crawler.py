...
catalog_path = os.path.join(
    os.path.dirname(__file__), "backend", "autovit_catalog.json"
)
try:
    ...
    log.info(
        "Loaded catalog",
        extra={
            "brands": len(BRANDS_AND_MODELS),
            "models": sum(len(m) for m in BRANDS_AND_MODELS.values()),
        },
    )
...
async def scrape_and_classify(make, possible_models):
    ...
    try:
            ...
            try:
                ...
                async def get_all_cars_sharded(m_make, m_model, min_y=1950, max_y=2026):
                    ...
                    if len(res) >= 800 and not is_github_actions and min_y < max_y:
                        ...
                        res2 = await get_all_cars_sharded(
                            m_make, m_model, mid_y + 1, max_y
                        )
                ...
                for car in results:
                    try:
                        ...
                        log.error(
                            "Car parse error",
                            extra={"link": car.get("link", "")[:60], "error": str(e)},
                        )
                ...
                if valid_cars:
                    try:
                        ...
                        if not is_github_actions:
                            ghosts = car_db_optimizer.mark_ghost_ads_inactive(
                                make, model, buffer_hours=12
                            )
                            if ghosts > 0:
                                log.info(
                                    "Ghost ads marked inactive",
                                    extra={
                                        "make": make,
                                        "model": model,
                                        "ghosts": ghosts,
                        ...
                        log.error(
                            "DB batch upsert failed",
                            extra={"make": make, "model": model, "error": str(db_err)},
                        )
                        for c in valid_cars:
                            try:
                                dead_letter.save(
                                    c, error=str(db_err), source="start_crawler"
                                )
                            except Exception:
                                pass
                ...
                log.error(
                    "Model scrape failed",
                    extra={"make": make, "model": model, "error": str(model_err)},
                )
                import traceback
        ...
        log.info(
            ...
            extra={"make": make, "saved": saved_count, "found": total_found},
        )
    except Exception as e:
